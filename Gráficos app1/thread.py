...
def load_ui_thread(path):
    """Lê um JSON e retorna a série da UI Thread CPU + tempo."""
...
plt.title("Comparativo – UI Thread CPU Usage – Aplicativo 1")
plt.xlabel("Tempo (ms)")
plt.ylabel("CPU (%) – UI Thread")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("comparativo_app1_UI.png")
plt.close()

print("Todos os gráficos do App1 foram gerados com sucesso!")
